...
<table>
    ...
    $query = mysqli_query($connect, "SELECT * FROM employee INNER JOIN position ON employee.position_id = position.id INNER JOIN dismissal_reason ON employee.dismissal_reason_id = dismissal_reason.id WHERE DATE_ADD(employee.start_date, INTERVAL 3 MONTH ) > CURRENT_DATE ORDER BY `employee`.`first_name` ASC");
